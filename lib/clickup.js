...
/**
 * 태스크에 댓글 추가. 멘션은 설명(description)에선 안 되고 댓글에서만 실제로 걸린다.
 * @param {string} taskId
 * @param {string} token
 * @param {Array<{text?:string, type?:string, user?:{id:number}}>} comment
 *        예: [{ text: '확인 부탁 ' }, { type: 'tag', user: { id: 123 } }]
 * @returns {Promise<Object>}
 */
export async function addTaskComment(taskId, token, comment) {
  if (!token) throw makeError(401, ERROR_MESSAGES[401]);
  if (!taskId) throw makeError(400, '댓글을 달 태스크를 찾을 수 없습니다.');
  return apiFetch(`/task/${encodeURIComponent(taskId)}/comment`, {
    token,
    method: 'POST',
    jsonBody: { comment },
  });
...
/**
 * 태스크 생성 + 첨부 (+ 선택적 멘션 댓글)를 한 번에. 패널에서 호출하는 상위 헬퍼.
 * @param {Object} params
 ...
 * @param {number} [params.mentionUserId] 지정 시 등록 후 해당 유저를 @멘션하는 댓글을 단다
 * @param {string} [params.mentionText] 멘션 앞에 붙일 문구 (선택)
 * @returns {Promise<{taskId:string, taskUrl:string, mentionOk:boolean}>}
 */
export async function submitReport({ token, listId, task, blob, filename, mentionUserId, mentionText }) {
  ...
  let mentionOk = true;
  if (mentionUserId) {
    // 멘션 댓글 실패가 태스크 생성 자체를 실패로 만들지 않도록 격리.
    try {
      const comment = [];
      if (mentionText) comment.push({ text: mentionText });
      comment.push({ type: 'tag', user: { id: mentionUserId } });
      await addTaskComment(created.id, token, comment);
    } catch {
      mentionOk = false;
    }
  }

  return { taskId: created.id, taskUrl: created.url, mentionOk };
}
