 ...
 * @returns {Promise<Array<{id:string, name:string}>>}
 */
export async function getSpaces(teamId, token) {
  const data = await apiFetch(`/team/${encodeURIComponent(teamId)}/space?archived=false`, { token });
  return data?.spaces || [];
}

/**
 * 리스트 단건 조회. 등록 대상을 ID 대신 이름으로 보여줄 때 쓴다.
 * @param {string} listId
 * @param {string} token
 * @returns {Promise<{id:string, name:string, folder?:{name:string, hidden?:boolean}, space?:{name:string}}>}
 */
export async function getList(listId, token) {
  if (!token) throw makeError(401, ERROR_MESSAGES[401]);
  return apiFetch(`/list/${encodeURIComponent(listId)}`, { token });
}

/** 등록 대상 경로를 이어 붙일 때 쓰는 구분자. */
export const LIST_PATH_SEP = ' - ';

/**
 * 리스트를 [스페이스, 폴더, 리스트] 조각으로. 리스트 이름만으로는 어느 스페이스인지
 * 알 수 없어 같은 이름이 여러 곳에 있으면 구분이 안 된다.
 *
 * 완성된 문자열이 아니라 조각을 돌려주는 이유: 이 값이 로컬에 캐시되는데,
 * 구분자를 바꿀 때마다 캐시된 문자열이 낡아 캐시를 무효화해야 하기 때문이다.
 *
 * 폴더 없이 스페이스 직속인 리스트에는 ClickUp이 hidden:true인 가짜 폴더를 끼워 넣으므로 걸러낸다.
 * @param {{name?:string, folder?:{name?:string, hidden?:boolean}, space?:{name?:string}}} list
 * @returns {string[]}
 */
export function listPathParts(list) {
  if (!list) return [];
  return [list.space?.name, list.folder && !list.folder.hidden ? list.folder.name : null, list.name].filter(
    Boolean,
  );
}

/**
 * 캐시된 조각을 표시용 문자열로. 조각이 아니면(구버전 캐시의 완성 문자열 등) 빈 값을
 * 돌려 호출부가 다시 조회하게 만든다.
 * @param {string[]} parts
 * @returns {string}
 */
export function joinListPath(parts) {
  return Array.isArray(parts) ? parts.filter(Boolean).join(LIST_PATH_SEP) : '';
}
 ...
 * @returns {Promise<Array<{id:string, name:string}>>}
 */
export async function getFolders(spaceId, token) {
  const data = await apiFetch(`/space/${encodeURIComponent(spaceId)}/folder?archived=false`, { token });
  return data?.folders || [];
}
 ...
 * @returns {Promise<Array<{id:string, name:string}>>}
 */
export async function getFolderLists(folderId, token) {
  const data = await apiFetch(`/folder/${encodeURIComponent(folderId)}/list?archived=false`, { token });
  return data?.lists || [];
}

/**
 * 폴더에 속하지 않은 스페이스 직속 리스트 목록.
 * @param {string} spaceId
 * @param {string} token
 * @returns {Promise<Array<{id:string, name:string}>>}
 */
export async function getFolderlessLists(spaceId, token) {
  const data = await apiFetch(`/space/${encodeURIComponent(spaceId)}/list?archived=false`, { token });
  return data?.lists || [];
}

/**
 * 리스트에 태스크 생성.
 * markdownContent를 주면 markdown_content 필드로 보내 서식(H3, 굵게 등)이 렌더된다
 * (ClickUp: markdown_content가 description보다 우선).
 * @param {string} listId
 * @param {string} token
 * @param {{name:string, description?:string, markdownContent?:string, priority?:number, tags?:string[], assignees?:number[]}} task
...
export async function createTask(listId, token, task) {
  ...
  if (task.markdownContent) {
    body.markdown_content = task.markdownContent;
  } else {
    body.description = task.description || '';
  }
  if (Array.isArray(task.tags) && task.tags.length) body.tags = task.tags;
  if (Array.isArray(task.assignees) && task.assignees.length) body.assignees = task.assignees;
...
/**
 * 태스크 단건 조회. 커스텀 ID(예: IMX-10468)는 custom_task_ids + team_id 필요.
 * include_markdown_description=true로 마크다운 저장본도 함께 받는다.
 * @param {string} taskId
 * @param {string} token
 * @param {{teamId?:string}} [opts]
 * @returns {Promise<Object>}
 */
export async function getTask(taskId, token, opts = {}) {
  if (!token) throw makeError(401, ERROR_MESSAGES[401]);
  const params = new URLSearchParams({ include_markdown_description: 'true' });
  const isCustom = /[^0-9]/.test(taskId); // 숫자 외 문자 포함 → 커스텀 ID
  if (isCustom) {
    params.set('custom_task_ids', 'true');
    if (opts.teamId) params.set('team_id', opts.teamId);
  }
  return apiFetch(`/task/${encodeURIComponent(taskId)}?${params.toString()}`, { token });
}

/**
 * 태스크 생성 + 첨부를 한 번에. 패널에서 호출하는 상위 헬퍼.
 * @param {Object} params
 * @param {string} params.token
 * @param {string} params.listId
 * @param {{name:string, description?:string, markdownContent?:string, priority?:number, tags?:string[], assignees?:number[]}} params.task
